...
const emit = defineEmits(['close', 'bot-deleted']);
...
const submit = () => {
    ...
    form.delete(route('account.bot.destroy', obj), {
        onSuccess: () => {
            emit('close');
            emit('bot-deleted'); // Emit the project-deleted event
        }
    });
};
...
<template>
    <div class="fixed inset-0 flex items-center justify-center z-50">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 z-10 max-w-sm w-full">
            <h3 class="text-lg font-semibold mb-4">Confirm Deletion {{props.bot.name}}</h3>
